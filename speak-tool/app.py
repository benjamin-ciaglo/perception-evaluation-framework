...
def complete(proctor_name, battery_name, test_idx, question_idx):
	...
	if (proctor_name == 'turk'):
		...
		for i in range(0, n):
			filename = os.path.join(save_location, env, worker_id + "_" + ass_id + "_worker_recording.txt")
			payload['question_'+str(i)+'_rec'] = os.path.join(app_dir,'user-content',proctor_name,worker_id+"_"+ass_id+".wav")
			payload['question_'+str(i)+'_worker_recording'] = os.path.join(app_dir,'user-content',proctor_name,worker_id+"_"+ass_id+"_worker_recording.txt")
			with open(filename,'r') as f:
				payload['question_'+str(i)+'_worker_recording'] = f.read()
...
def submit(proctor_name, battery_name, test_idx):
	...
	for i in range(0, n):
		filename = os.path.join(save_location, env,worker_id + "_" + ass_id + "_worker_recording.txt")
		payload['question_'+str(i)+'_rec'] = os.path.join(app_dir,'user-content',proctor_name,worker_id+"_"+ass_id+".wav")
		payload['question_'+str(i)+'_worker_transcript_loc'] = os.path.join(app_dir,'user-content',proctor_name,worker_id+"_"+ass_id+"_worker_recording.txt")
		with open(filename,'r') as f:
			payload['question_'+str(i)+'_worker_recording'] = f.read()
		payload['questions_passed'] = payload['questions_passed'] + " " + str(session[ass_id + "_" + test_idx + "_" + str(i)])
	print('final payload:',payload)
